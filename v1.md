...
## 3 · Image Upload & Delivery Pipeline
...
## 5 · Webhook Reliability & Dead-Letter Flow
...
══════════════════════════════════════════════════════════════════════════════════
  ...
  acceptable replay patterns on Render:
    1. admin-triggered replay only
    2. Render Cron Job scans webhook_failures every N minutes
    3. separate background worker service consumes webhook_failures

  minimum recommended here:
    Render Web Service + PostgreSQL dead-letter table + admin retry endpoint
    add Render Cron Job if stale-cache MTTR must be automatic


WEBHOOK DELIVERY CONTRACT · exact request shape
══════════════════════════════════════════════════════════════════════════════════

  POST FRONTEND_REVALIDATE_URL
  headers:
    Authorization: Bearer {FRONTEND_REVALIDATE_TOKEN}
    Content-Type: application/json

  body:
    {
      "tenant_id": "acme",
      "paths": ["/posts/q3-recap", "/about"],
      "reason": "article.publish"
    }

  success criteria:
    - treat any 2xx as success
    - record response status/body on non-2xx
    - timeout counts as failed attempt
    - request must be idempotent; duplicate webhook must be safe


CACHE TAG CONTRACT · required for bulk tenant revalidation
══════════════════════════════════════════════════════════════════════════════════

  every tenant-scoped frontend fetch must include:
    tags: ["tenant:{tenant_id}"]

  every article page fetch should also include:
    tags: ["article:{article_id}", "tenant:{tenant_id}"]

  use:
    revalidatePath(path)           for single known page
    revalidateTag("tenant:acme")   for full-tenant refresh

  without this contract:
    "revalidate whole tenant" is undefined and becomes an ops-only manual task


OBSERVABILITY · enough to debug real incidents
══════════════════════════════════════════════════════════════════════════════════
  ...
  every RLS-sensitive DB transaction logs:
    request_id
    tenant_id
    tx_started
